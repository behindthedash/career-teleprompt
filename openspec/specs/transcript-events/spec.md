...
Each event SHALL belong to exactly one session identity. The dispatcher SHALL allocate a unique session identity per recording session and SHALL NOT relabel delayed prior-session events as current-session events. New sessions SHALL not inherit queued events or relabeled data from prior sessions.
