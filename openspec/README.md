...
Hearsay is the reusable Windows transcription host. It owns audio capture, local transcription, source-tagged transcript events, live-only session behavior, low-latency transcription profiles, extension-host import/lifecycle boundaries, and upstream-ready hardening.

Hearsay does **not** own interview question detection, resume/project RAG, knowledge-store schemas, interview cue generation, overlay/teleprompter UI infrastructure, teleprompter behavior, or target-company preparation. Those belong to a separate consumer project.
...
2. [`002-upstream-readiness.md`](epics/002-upstream-readiness.md) — clean import/dependency boundaries and contribution discipline.
...
- Importing the supported extension API must not start UI/audio side effects.
...
- Do not add domain-specific retrieval, personal-knowledge, or interview UI semantics to Hearsay core.
