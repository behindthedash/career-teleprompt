...
- CI is the required merge gate. Every PR to `dev` must pass the Windows matrix on Python 3.11 and 3.14, including `ruff check src tests scripts`, `ruff format --check src tests scripts`, and `pytest -q`.
- Verify changes locally with the same Ruff and pytest commands before opening or updating a PR. Before releasing anything that touches audio, also run `python scripts/manual_device_check.py` (~3 min; records real devices while playing TTS through the speakers).
...
- **Transcript events:** the application drain boundary converts finalized, source-labeled segments into immutable session-scoped `TranscriptEvent` values after overlap deduplication and echo suppression. Session identity and monotonic per-session sequence numbers are explicit; teardown drains retain their originating session identity. Subscriber registration/delivery is a separate extension-host layer built on this seam.
