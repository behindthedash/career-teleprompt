...
const ALL_TABS: TabItem[] = TAB_GROUPS.flatMap((group) => group.items);
const TAB_LABELS: Record<SettingsTab, string> = Object.fromEntries(
  ALL_TABS.map((tab) => [tab.id, tab.label]),
) as Record<SettingsTab, string>;

const tabDomId = (tab: SettingsTab) => `settings-tab-${tab}`;
const panelDomId = (tab: SettingsTab) => `settings-panel-${tab}`;
...
export function SettingsOverlay({ isModal = false }: SettingsOverlayProps) {
  ...
  const previousView = useMeetingStore((state) => state.previousView);
  ...
  const handleBackdropClick = useCallback(
    (event: React.MouseEvent) => {
      if (isModal && event.target === backdropRef.current) {
        handleCloseModal();
      }
    },
  ...
  const selectAndFocusTab = useCallback((tab: SettingsTab) => {
    setActiveTab(tab);
    requestAnimationFrame(() => document.getElementById(tabDomId(tab))?.focus());
  }, []);

  const handleTabKeyDown = useCallback(
    (event: React.KeyboardEvent<HTMLButtonElement>, tab: SettingsTab) => {
      const currentIndex = ALL_TABS.findIndex((item) => item.id === tab);
      ...
      selectAndFocusTab(ALL_TABS[nextIndex].id);
    },
    [selectAndFocusTab],
  );
    ...
    return (
      ...
      >
        ...
        >
          ...
          <div
            className="flex border-b border-border/30 px-2 overflow-x-auto"
            role="tablist"
          ...
          >
            {ALL_TABS.map((tab) => {
              ...
              return (
                ...
                >
                  <div
                    className={`absolute bottom-0 left-1 right-1 h-[2px] rounded-full bg-primary transition-all duration-200 ${
    ...
    <div className="flex h-full w-full bg-background">
        ...
        >
          {TAB_GROUPS.map((group, groupIndex) => (
            <div key={group.label} className={groupIndex > 0 ? "mt-4" : ""}>
              <div className="mb-1 px-3 flex items-center gap-2" aria-hidden="true">
              ...
              <div className="space-y-0.5">
                {group.items.map((tab) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`absolute left-0 top-1/2 -translate-y-1/2 rounded-r-full bg-primary transition-all duration-200 ease-out ${
      ...
      <main className="flex-1 overflow-y-auto">
        <section
          ...
          aria-labelledby={tabDomId(activeTab)}
          tabIndex={0}
          className={`mx-auto ${contentMaxW} px-8 py-8`}
        >
          <div className="mb-8">
